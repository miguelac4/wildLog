...
try {
    ...
    // Rate limit simples: não enviar se já enviaste há menos de 1h
    $stmt = $pdo->prepare("
        SELECT created_at
        FROM user_tokens
        WHERE user_id = ? AND type = 'resetPass'
    ...
    if ($last) {
        ...
        if ($lastTs !== false && (time() - $lastTs) < 3600) {
            api_json_error(429, 'TOO_MANY_REQUESTS', 'Aguarde antes de pedir novo email.');
        }
    }
    ...
    UPDATE user_tokens
    SET consumed_at = NOW()
    WHERE user_id = ? AND type = 'resetPass' AND consumed_at IS NULL
");
    $stmt->execute([(int)$user["id"]]);
...
");
    ...
    // URL verificação (GET)
    $verifyUrl = "https://wild-log.com/backend/api/auth/pass_recover_confirm.php?token=" . $token;
...
<head>
  ...
  <title>Password Reset - WildLog</title>
</head>
<body style='margin:0;padding:0;background:#f4f4f4;font-family:Arial, sans-serif;color:#333;'>
  <div style='max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.10);'>
    ...
    <!-- Content -->
    <div style='padding:42px 30px;background:#ffffff;'>
      <h1 style='
        margin:0 0 16px 0;
        font-size:30px;
        font-weight:700;
      ...
      <p style='margin:0 0 14px 0;font-size:16px;line-height:1.8;text-align:center;color:#555555;'>
        Hello <strong style='color:#c0a16b;'>".$safeName."</strong>,
      </p>

      <p style='margin:0 0 22px 0;font-size:16px;line-height:1.8;text-align:center;color:#555555;'>
        We received a request to reset the password for your <strong>WildLog</strong> account.
      </p>
      ...
      <div style='background:#f9f9f9;border-left:4px solid #c0a16b;padding:16px;border-radius:0 8px 8px 0;'>
        <p style='margin:0;font-size:13px;line-height:1.6;color:#666666;'>
          <strong>Security note:</strong> This link is valid for <strong>24 hours</strong> and can only be used once.
          If you didn’t request a password reset, you can safely ignore this email — your account remains secure.
        </p>
      </div>

      <p style='margin:20px 0 0 0;font-size:13px;line-height:1.6;text-align:center;color:#777777;'>
        See you around the campfire,<br/>
        <span style='color:#c0a16b;font-weight:700;'>The WildLog Team</span>
      </p>
    </div>

    <!-- Footer -->
    <div style='background:#f8f8f8;padding:24px 30px;text-align:center;border-top:1px solid #eeeeee;'>
...
";
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Reset your password - WildLog';
    $mail->Body = $emailTemplate;
    $mail->AltBody =
        "Reset your password - WildLog

Hello $name,

We received a request to reset your WildLog account password.
To set a new password, open the link below (valid for 24 hours):
...
If you didn’t request this, simply ignore this email.";
    ...
    api_log_exception($e, $requestId, [
        'endpoint' => '.../auth/pass_recover_request.php',
    ]);
